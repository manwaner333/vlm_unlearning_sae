...
def save_sae_activations(sparse_autoencoder, model, dataset_path, label_index, label, directory):
    ...
    while number_of_images_processed < len(dataset):
        ...
        torch.save(sae_activations, f'{directory}/sae_activations/color/sae_activations_{label_index[label]}.pt')
        number_of_images_processed += max_number_of_images_per_iteration
        

def save_selected_features(directory, label, compare_label, k):
    sae_activations_0 = torch.load(f'{directory}/sae_activations/color/sae_activations_{label}.pt').to('cpu')
    sae_activations_200 = torch.load(f'{directory}/sae_activations/color/sae_activations_{compare_label}.pt').to('cpu') 
    ...
    torch.save(indices, f'{directory}/feature_indices/color/feature_indices_{label}.pt')
    torch.save(values, f'{directory}/feature_values/color/feature_values_{label}.pt')
...
def generate_adj_sae_outputs(sparse_autoencoder, model, directory, label, max_token):
    image_file = f"{directory}/images/color/image_{label}.jpg"
    raw_image = Image.open(image_file)
    
    res_file = f"{directory}/color_adj_sae_outputs.json"
    ...
    conversation = [
        ...
        "role": "user",
        "content": [
            {"type": "text", "text": "Please describe the objects in the image and their corresponding colors."},   # "What are these?" "Please describe this picture." What is the main color of the object in this picture?
            {"type": "image"},
            ],
        },
    ]
    ...
    print(output_texts)
    res = {"label": label, "instruction": "Please describe the objects in the image and their corresponding colors.", "output_texts": output_texts}
...
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 0}
#     elif "blue" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 1}
#     elif "green" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 2}
#     elif "yellow" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 3}
#     elif "purple" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 4}
#     elif "orange" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 5}
#     elif "pink" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 6}
#     elif "brown" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 7}
#     elif "black" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 8}
#     elif "white" in file.lower():
#         ele = {'image': f'dataset/color/{file.lower()}', "label": 9}
#     res.append(ele)
# res_file= f"dataset/color.json"
...
# data_path = f"dataset/color.json"
# label_index = {"red":0, "blue":1, "green":2, "yellow":3, "purple":4, "orange":5, "pink":6, "brown":7, "black":8, "white":9}
# for label in tqdm(["red", "blue", "green", "yellow", "purple", "orange", "pink", "brown", "black", "white"], desc="Processing labels"):
#     file_path = f'{directory}/sae_activations/color/sae_activations_{label_index[label]}.pt'
