...
def save_sae_activations(sparse_autoencoder, model, dataset_path, split, label, seed, directory):
    ...
    while number_of_images_processed < len(dataset):
        ...
        torch.save(sae_activations, f'{directory}/sae_activations/sae_activations_{label}.pt')
...
def save_figures(sparse_autoencoder, directory, dataset_path, split, label):
    ...
    # for i in range(0, 10):
    #     first_image = dataset[i][image_key]
    #     first_image.save(f"{directory}/images/bird/image_{label}_{i}.png")
...
### 3. save figures
# for label in tqdm(range(140, 142, 2), desc="Processing Figures"):
