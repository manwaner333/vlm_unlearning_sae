...
def save_sae_activations(sparse_autoencoder, model, dataset_path, label_index, label, directory):
    ...
    while number_of_images_processed < len(dataset):
        ...
        torch.save(sae_activations, f'{directory}/sae_activations/sport/sae_activations_{label_index[label]}.pt')
        number_of_images_processed += max_number_of_images_per_iteration
        

def save_selected_features(directory, label, compare_label, k):
    sae_activations_0 = torch.load(f'{directory}/sae_activations/sport/sae_activations_{label}.pt').to('cpu')
    sae_activations_200 = torch.load(f'{directory}/sae_activations/sport/sae_activations_{compare_label}.pt').to('cpu') 
    ...
    torch.save(indices, f'{directory}/feature_indices/sport/feature_indices_{label}.pt')
    torch.save(values, f'{directory}/feature_values/sport/feature_values_{label}.pt')
...
def generate_adj_sae_outputs(sparse_autoencoder, model, directory, label, max_token):
    ...
    res_file = f"{directory}/sport_adj_sae_outputs.json"
    ...
    conversation = [
        ...
        "role": "user",
        "content": [
            {"type": "text", "text": "Please identify the sport depicted in the image by selecting one of the following categories: [soccer, basketball, football, tennis, baseball, volleyball, golf, hockey, rugby, badminton]."},   # "What are these?" "Please describe this picture." What is the main color of the object in this picture?
            {"type": "image"},
            ],
        },
    ]
...
# res = []
# for file in files:
#     if "soccer" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 0}
#     elif "basketball" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 1}
#     elif "football" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 2}
#     elif "tennis" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 3}
#     elif "baseball" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 4}
#     elif "volleyball" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 5}
#     elif "golf" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 6}
#     elif "hockey" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 7}
#     elif "rugby" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 8}
#     elif "badminton" in file.lower():
#         ele = {'image': f'dataset/sport/{file.lower()}', "label": 9}
#     res.append(ele)
# res_file= f"dataset/sport.json"
...
# data_path = f"dataset/sport.json"
# label_index = {"soccer":0, "basketball":1, "football":2, "tennis":3, "baseball":4, "volleyball":5, "golf":6, "hockey":7, "rugby":8, "badminton":9}
# for label in tqdm(["soccer", "basketball", "football", "tennis", "baseball", "volleyball", "golf", "hockey", "rugby", "badminton"], desc="Processing labels"):
#     file_path = f'{directory}/sae_activations/sport/sae_activations_{label_index[label]}.pt'
