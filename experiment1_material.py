...
def save_sae_activations(sparse_autoencoder, model, dataset_path, label_index, label, directory):
    ...
    while number_of_images_processed < len(dataset):
        ...
        torch.save(sae_activations, f'{directory}/sae_activations/material/sae_activations_{label_index[label]}.pt')
        number_of_images_processed += max_number_of_images_per_iteration
        

def save_selected_features(directory, label, compare_label, k):
    sae_activations_0 = torch.load(f'{directory}/sae_activations/material/sae_activations_{label}.pt').to('cpu')
    sae_activations_200 = torch.load(f'{directory}/sae_activations/material/sae_activations_{compare_label}.pt').to('cpu') 
    ...
    torch.save(indices, f'{directory}/feature_indices/material/feature_indices_{label}.pt')
    torch.save(values, f'{directory}/feature_values/material/feature_values_{label}.pt')
...
def generate_adj_sae_outputs(sparse_autoencoder, model, directory, label, max_token):
    image_file = f"{directory}/images/material/image_{label}.jpg"
    raw_image = Image.open(image_file)
    
    res_file = f"{directory}/material_adj_sae_outputs.json"
    ...
    conversation = [
        ...
        "role": "user",
        "content": [
            {"type": "text", "text": "Please determine the primary material of the item shown in the image by selecting one of the following categories: [wood, metal, plastic, glass, fabric, paper, ceramic, leather, concrete, stone]."},   # "What are these?" "Please describe this picture." What is the main color of the object in this picture?
            {"type": "image"},
            ],
        },
    ]
    ...
    print(output_texts)
    res = {"label": label, "instruction": "Please determine the primary material of the item shown in the image by selecting one of the following categories: [wood, metal, plastic, glass, fabric, paper, ceramic, leather, concrete, stone].", "output_texts": output_texts}
...
# res = []
# for file in files:
#     if "wood" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 0}
#     elif "metal" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 1}
#     elif "plastic" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 2}
#     elif "glass" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 3}
#     elif "fabric" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 4}
#     elif "paper" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 5}
#     elif "ceramic" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 6}
#     elif "leather" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 7}
#     elif "concrete" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 8}
#     elif "stone" in file.lower():
#         ele = {'image': f'dataset/material/{file.lower()}', "label": 9}
#     res.append(ele)
# res_file= f"dataset/material.json"
...
# data_path = f"dataset/material.json"
# label_index = {"wood":0, "metal":1, "plastic":2, "glass":3, "fabric":4, "paper":5, "ceramic":6, "leather":7, "concrete":8, "stone":9}
# for label in tqdm(["wood", "metal", "plastic", "glass", "fabric", "paper", "ceramic", "leather", "concrete", "stone"], desc="Processing labels"):
#     file_path = f'{directory}/sae_activations/material/sae_activations_{label_index[label]}.pt'
